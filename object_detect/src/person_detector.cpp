...
protected:
    ...
    void personsDetect(const cv::Mat& img,
    ...
    boost::scoped_ptr<cv::HOGDescriptor> _hog;
...
{  
  ...
  _imageSub   = _imageTransport.subscribe(topic, 1, &PersonDetector::imgCallback, this, transportHint);
//   _imDebugPub = _privateImageTransport.advertise("debug", 1);

  _detectionPub = _pnh.advertise<object_recognition_msgs::ObjectType>("detections", 1);
...
void PersonDetector::imgCallback(const sensor_msgs::ImageConstPtr& msg)
{
  ...
  cv::Mat img(static_cast<int>(_imgScaling*cvImgPtr->image.rows),
              static_cast<int>(_imgScaling*cvImgPtr->image.cols),
              cvImgPtr->image.type());
...
void PersonDetector::publishDetections(const std::vector<cv::Rect>& detections) const
{
  ...
  BOOST_FOREACH(const cv::Rect& roi)
  {
    msg.pose.pose.pose.position.x      = roi.x;
    msg.pose.pose.pose.position.z      = roi.y;
  }
